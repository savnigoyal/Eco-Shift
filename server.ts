...
app.post("/api/gemini/insight", async (req, res) => {
  const { transport_mode, transport_distance, ac_hours, laptop_hours, diet_type, digital_hours } = req.body;

  if (!ai) {
    return res.json({ 
      insight: `Based on your habits (Transport: ${transport_mode} for ${transport_distance}km, AC: ${ac_hours}hrs, Laptop: ${laptop_hours}hrs, Diet: ${diet_type}, Digital: ${digital_hours}hrs), your transport mode is your biggest potential card to play. Switching to zero-carbon modes or public transport like metro just twice weekly can drastically lower your carbon footprint. Continue tracking to see the projected impact!`
    });
  }
